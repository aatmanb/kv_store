...
int populateDB(std::string target_str, std::string fname, bool crash_consistency_test) {
    ...
    std::cout << "Connecting to target: " << target_str << "\n";
    kv739_init(target_str);
            ...
            if (status == -1) {
                ...
                if (num_retry == max_retry) {
                    if (!crash_consistency_test) {
                        ...
                        num_retry = 0;
                        // Re-initialize the connection to the server
                        kv739_init(target_str);
                    }
                }
                num_retry++;
...
int main(int argc, char** argv) {
    ...
    auto id_string = parser.getCmdOption("--id");
    std::cout << id_string << "\n";
    int id=0;
    // int id = std::stoi(id_string);
    std::string target_str = parser.getCmdOption("--target");
    std::string real_fname = parser.getCmdOption("--real");
    std::string fake_fname = parser.getCmdOption("--fake");
    int32_t test_type = 1;//std::stoi(parser.getCmdOption("--test-type"));
