...
$BIN_DIR/keyvaluestore_server >| $OUTPUT_DIR/server.log 2>&1 &
...
do
    args="--id $i --target localhost:50051 --real $TEST_DIR/data/real$i.csv --fake $TEST_DIR/data/fake$i.csv"
...
if [[ "$TEST_TYPE" == "--crash-consistency-test" ]]; then
    ...
    do
        ...
        echo "Starting server"
        # Start the server in the background and redirect output to server.log
        $BIN_DIR/keyvaluestore_server >| $OUTPUT_DIR/server$i.log 2>&1 &
        # Store the server process ID to kill it later if needed
        SERVER_PID=$!
        echo "Server PID $SERVER_PID"
    done
